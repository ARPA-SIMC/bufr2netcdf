...
dnl Try with known env vars
if ! test -z "$NETCDF_INCLUDE" && ! test -z "$NETCDF_LIB"
then
	have_netcdf=yes
	NETCDF_CFLAGS="$NETCDF_INCLUDE"
	NETCDF_LIBS="$NETCDF_LIB"
else
	dnl Try pkg-config
	PKG_CHECK_EXISTS([netcdf], [have_netcdf=yes], [have_netcdf=no])
	if test $have_netcdf = yes; then
	    PKG_CHECK_MODULES(NETCDF,netcdf)
	fi

	dnl Else try to find the header in several possible locations
